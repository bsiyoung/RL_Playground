...
def run() -> None:
    ...
    for curr_step in range(1, n_step + 1):
        # Select an action
        if 0 in cnt_action:
            action = np.where(cnt_action == 0)[0][0]
        else:
            q_upper_bound = q + confidence_level * np.sqrt(np.log(curr_step) / cnt_action)
            action = int(np.argmax(q_upper_bound))
